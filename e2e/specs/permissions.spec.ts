...
test.describe('Admin-only API operations', () => {
  ...
  test('GET /api/admin/stats returns 401/403 for non-admin', async ({ browser }) => {
    ...
    // Explicit empty storageState to guarantee no cookie inheritance from any prior context.
    const ctx = await browser.newContext({ storageState: { cookies: [], origins: [] } });
    const page = await ctx.newPage();
    await registerViaUI(page, u.email, u.username, u.password);

    // Sanity-check: page.request uses the browser's internal request context which
    // may not share cookies set via browser navigation in all Playwright builds.
    // Use page.evaluate(fetch) so the browser's own cookie jar is used instead.
