...
- Name: auth.spec.ts >> Login lockout >> shows remaining attempts warning after wrong passwords
- Location: specs/auth.spec.ts:98:7
...
> 89  |   test.beforeAll(async ({ browser }) => {
  ...
  117 |     // Should not be redirected to login
