              ...
              {emailStatus?.enabled && (
                <button
                  onClick={() => {
                    ...
                    api.emailConfig.clear()
                      .then(() => api.emailStatus.get())
                      .then((s) => { setEmailStatus(s); setShowSmtpForm(true); })
                      .catch((e) => showToast((e as Error).message, 'error'));
                  }}
              ...
              {verifyEmailPrompt && (
                <div className="flex items-start gap-3 px-4 py-3 rounded-xl" style={{ background: '#f59e0b15', border: '1px solid #f59e0b44' }}>
                  ...
                  <div className="flex-1 text-sm" style={{ color: 'var(--text-2)' }}>
                    ...
                    We sent a verification link to <strong>{currentUser?.email}</strong>. Click it, then come back and enable this setting.
                  </div>
