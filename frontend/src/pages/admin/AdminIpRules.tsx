...
function RuleList({
  ...
  async function add() {
    ...
    setAdding(true);
    try { await onAdd(cidr.trim(), desc.trim()); setCidr(''); setDesc(''); }
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        <input className="input text-sm" style={{ width: 180 }} placeholder="1.2.3.4 or 10.0.0.0/8"
          value={cidr} onChange={(e) => setCidr(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') add(); }} />
        <input className="input text-sm flex-1" placeholder="Description (optional)"
      ...
      {rules.length === 0 ? (
        <p className="text-xs" style={{ color: 'var(--text-3)' }}>No rules.</p>
      ) : (
        <div className="space-y-1">
          {rules.map((r) => (
            <div key={r.id} className="flex items-center gap-3 px-4 py-2.5 rounded-xl"
              style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
              <span className="text-sm font-mono" style={{ color: 'var(--text)' }}>{r.cidr}</span>
              {r.description
                ? <span className="text-xs flex-1 min-w-0 truncate" style={{ color: 'var(--text-3)' }}>{r.description}</span>
                : <span className="flex-1" />}
              <span className="text-xs flex-shrink-0" style={{ color: 'var(--text-3)' }}>{new Date(r.createdAt).toLocaleDateString()}</span>
              <button onClick={() => onRemove(r.id).catch((e) => showToast((e as Error).message, 'error'))}
    ...
    <div className="space-y-10 max-w-xl">
      <div>
        ...
        <IpRulesPanel
          ...
          data={userIp}
          onAddRule={async (cidr, listType, desc) => { await api.admin.addIpRule(cidr, listType, desc || undefined); await load(); showToast('Rule added', 'success'); }}
      ...
      <div style={{ borderTop: '1px solid var(--border)', paddingTop: '2rem' }}>
        ...
        <p className="text-xs mb-4" style={{ color: 'var(--text-3)' }}>
          Restricts admin and server owner access to <code style={{ background: 'var(--surface)' }} className="px-1 rounded">/api/admin/*</code> routes.
          Use this to limit admin actions to specific IPs — for example, your home network or a company IP range.
        </p>
        <IpRulesPanel
          title="IP rules — admins only"
          description="Regular users are unaffected by these rules."
          data={adminIp}
          onAddRule={async (cidr, listType, desc) => { await api.admin.addAdminIpRule(cidr, listType, desc || undefined); await load(); showToast('Rule added', 'success'); }}
