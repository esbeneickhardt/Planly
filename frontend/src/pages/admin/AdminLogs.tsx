  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-end gap-2 flex-wrap">
        ...
        <button
          onClick={() => fetchLogs({ action: logAction || undefined, from: logFrom || undefined, to: logTo || undefined })}
          className="px-3 py-1.5 rounded-lg text-sm font-medium"
          style={{ background: 'var(--brand)', color: 'white' }}>
          Apply
        </button>
        <button
          onClick={() => { setLogAction(''); setLogFrom(''); setLogTo(''); fetchLogs({}); }}
        ...
        <div className="flex gap-2 ml-auto">
          <button
            onClick={() => api.admin.exportLogs({ format: 'csv', action: logAction || undefined, from: logFrom || undefined, to: logTo || undefined }).catch((e) => showToast((e as Error).message, 'error'))}
            className="px-3 py-1.5 rounded-lg text-xs font-medium"
            style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text-2)' }}>
            ↓ CSV
          </button>
          <button
            onClick={() => api.admin.exportLogs({ format: 'jsonl', action: logAction || undefined, from: logFrom || undefined, to: logTo || undefined }).catch((e) => showToast((e as Error).message, 'error'))}
            className="px-3 py-1.5 rounded-lg text-xs font-medium"
            style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text-2)' }}>
            ↓ JSONL
          </button>
      ...
      <div className="space-y-1">
        {logs.length === 0 ? (
          ...
          <>
            ...
            {hasMoreLogs && (
              <button
                onClick={() => fetchLogs({ cursor: logCursor ?? undefined, action: logAction || undefined, from: logFrom || undefined, to: logTo || undefined, append: true })}
