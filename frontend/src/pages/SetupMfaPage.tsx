...
export default function SetupMfaPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-sm p-8 rounded-2xl shadow-lg" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div className="flex items-center gap-3 mb-6">
          <div className="w-9 h-9 rounded-xl overflow-hidden flex-shrink-0">
            <img src="/icons/icon.jpg" alt="Planly" className="w-full h-full object-cover" />
          </div>
          <div>
            <h1 className="text-base font-bold" style={{ color: 'var(--text)' }}>Set up two-factor authentication</h1>
            <p className="text-xs" style={{ color: 'var(--text-3)' }}>Required by your administrator</p>
          </div>
        </div>

        {error && (
          <div className="mb-4 text-sm p-3 rounded-lg" style={{ background: '#ef444418', border: '1px solid #ef444444', color: '#ef4444' }}>{error}</div>
        )}

        {step === 'start' && (
          <div className="space-y-4">
            <div className="p-3 rounded-lg text-xs" style={{ background: '#6366f118', border: '1px solid #6366f144', color: 'var(--text-2)' }}>
              This server requires all users to protect their account with an authenticator app (Google Authenticator, Authy, 1Password, etc.). You'll need your app before you can continue.
            </div>
            <button onClick={handleStart} disabled={loading} className="btn-primary w-full justify-center flex">
              {loading ? <span className="inline-block w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /> : 'Begin setup →'}
            </button>
        ...
        {step === 'scan' && (
          <div className="space-y-4">
            ...
            <button onClick={handleConfirm} disabled={loading || confirmCode.length < 6} className="btn-primary w-full justify-center flex">
