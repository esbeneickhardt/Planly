...
function actionLabel(action: string) {
  const map: Record<string, string> = {
    ...
    'cycle.created': 'created sprint',
    'cycle.updated': 'updated sprint',
  };
...
export default function AnalyticsPage() {
  ...
  const [cumulativePeriod, setCumulativePeriod] = useState<Period>(
    validPeriod(savedPrefs.cumulativePeriod) ? savedPrefs.cumulativePeriod : 'all',
  );
  const [weekdayPeriod, setWeekdayPeriod] = useState<Period>(
    validPeriod(savedPrefs.weekdayPeriod) ? savedPrefs.weekdayPeriod : '30d',
  );
  ...
  const loadAnalytics = useCallback(async (productId: string) => {
    ...
    setError(null);
    try {
      setData(await api.analytics.get(productId));
    } catch (e) {
  ...
  const loadWorkload = useCallback(async (productId: string) => {
    try {
      setWorkload(await api.analytics.workload(productId));
    } catch {
      /* non-critical workload data */
    }
  }, []);
  ...
  const cumulativeDays =
    cumulativePeriod === 'all' ? allDays : allDays.slice(allDays.length - PERIOD_DAYS[cumulativePeriod]);
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
              ...
              {(() => {
                ...
                const activeSprint = data.sprintVelocity.find(
                  (s) => s.startDate.slice(0, 10) <= today && s.endDate.slice(0, 10) >= today,
                );
                      ...
                      <p className="text-xs mt-0.5" style={{ color: 'var(--text-3)' }}>
                        ...
                        {activeSprint && (
                          <span
                            ...
                            style={{
                              background: `${activeSprint.color ?? 'var(--brand)'}22`,
                              color: activeSprint.color ?? 'var(--brand)',
                              border: `1px solid ${activeSprint.color ?? 'var(--brand)'}44`,
                            }}
                          >
                            <span
                              style={{
                                ...
                                height: 5,
                                borderRadius: '50%',
                                background: activeSprint.color ?? 'var(--brand)',
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs mt-0.5" style={{ color: 'var(--text-3)' }}>
                      Running total of tasks finished over time
                    </p>
                  </div>
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs mt-0.5" style={{ color: 'var(--text-3)' }}>
                      Which days the team tends to ship
                    </p>
                  </div>
            ...
            {data.sprintVelocity.length > 0 && (
              ...
              >
                ...
                <p className="text-xs mb-4" style={{ color: 'var(--text-3)' }}>
                  How many tasks were finished within each sub-plan's dates
                </p>
          ...
          <div className="rounded-xl p-6 shadow-sm" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: 'var(--text-3)' }}>
                  {user?.realName ?? user?.username} · {workload.totalActive} active · {workload.totalCompleted}{' '}
                  completed
                </p>
              </div>
            </div>
                        ...
                        <div key={status}>
                          ...
                          <div className="h-1 rounded-full overflow-hidden" style={{ background: 'var(--surface-2)' }}>
                            <div
                              className="h-full rounded-full"
                              style={{
                                width: `${(count / workload.totalActive) * 100}%`,
                                background: STATUS_COLOR[status] ?? '#64748b',
                                opacity: 0.7,
                              }}
                            />
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs leading-relaxed" style={{ color: 'var(--text)' }}>
                      <span className="font-medium">
                        {(ev.metadata as { actorName?: string })?.actorName ?? 'Unknown'}
                      </span>{' '}
      ...
      {sprints.map((s) => (
        <div key={s.sprintId} className="flex items-center gap-3 group">
          ...
          <div style={{ width: 148, flexShrink: 0 }}>
            ...
            <div className="text-[10px] leading-snug" style={{ color: 'var(--text-3)' }}>
          ...
          <div className="flex-1 relative h-5 rounded-full overflow-hidden" style={{ background: 'var(--surface-2)' }}>
            <div
              ...
              style={{
                width: `${(s.completed / max) * 100}%`,
                background: s.color || 'var(--brand)',
                opacity: 0.65,
                minWidth: s.completed > 0 ? 6 : 0,
              }}
            />
          </div>
          <div className="text-xs font-medium flex-shrink-0 w-7 text-right" style={{ color: 'var(--text-3)' }}>
            {s.completed}
          </div>
        </div>
      ))}
      ...
      {(['7d', '30d', '90d', 'all'] as Period[]).map((p) => (
        <button
          ...
          style={{
            ...
            color: value === p ? 'var(--text)' : 'var(--text-3)',
