      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-4 md:py-6">
        {loading ? (
          ...
          <>
            ...
            {tab === 'email' && (
              <AdminEmail currentUser={me} refreshUser={refreshUser} onUsersChanged={loadUsers} showToast={showToast} />
            )}
            {tab === 'ip-rules' && <AdminIpRules showToast={showToast} />}
            {tab === 'logs' && <AdminLogs isFoundingAdmin={!!me.isFoundingAdmin} showToast={showToast} />}
            {tab === 'statistics' && <AdminStats users={users} showToast={showToast} />}
          </>
        )}
      </div>
    </div>
