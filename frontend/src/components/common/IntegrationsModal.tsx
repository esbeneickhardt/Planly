...
export default function IntegrationsModal({ onClose }: Props) {
  ...
  useEffect(() => {
    api.apiTokens.list().then(setTokens).catch(() => {}).finally(() => setLoading(false));
  }, []);

  useEffect(() => {
    if (tab !== 'permissions' || perms.length > 0) return;
    setPermsLoading(true);
    api.me.permissions().then(setPerms).catch(() => {}).finally(() => setPermsLoading(false));
  }, [tab]);
      ...
      {tab === 'permissions' && (
        <div className="space-y-2 mb-2">
          ...
          {permsLoading ? (
            ...
            <p className="text-sm py-4 text-center" style={{ color: 'var(--text-3)' }}>No projects found.</p>
          ) : (
            perms.map((p) => (
                ...
                {Object.keys(p.permissions).length > 0 ? (
                  <div className="flex flex-wrap gap-1.5">
                    {Object.entries(p.permissions).map(([permTab, level]) => (
                      <span key={permTab} className="text-[11px] px-2 py-0.5 rounded-full" style={{
                  ...
                  <p className="text-[11px]" style={{ color: 'var(--text-3)' }}>Full access (no restrictions set)</p>
                )}
              </div>
            ))
          )}
      ...
      {tab === 'tokens' && (<>
          ...
          <div className="mb-5 rounded-xl p-3 space-y-2" style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.25)' }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                style={{ background: copied ? 'rgba(16,185,129,0.15)' : 'var(--brand)', color: copied ? '#10b981' : 'white' }}
              >
                {copied ? 'Copied!' : 'Copy'}
              </button>
            </div>
            <button onClick={() => setRevealed(null)} className="text-[11px]" style={{ color: 'var(--text-3)' }}>Dismiss</button>
          </div>
        )}

        {/* Create form */}
        <form onSubmit={handleCreate} className="mb-5 space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--text-3)' }}>New token</p>
        ...
        <div className="space-y-2">
          ...
          {loading ? (
            ...
            <p className="text-sm py-4 text-center" style={{ color: 'var(--text-3)' }}>No tokens yet.</p>
          ) : (
            tokens.map((t) => {
              ...
              return (
                <div
                  ...
                  style={{ background: 'var(--surface-2)', border: `1px solid ${expired ? 'rgba(239,68,68,0.25)' : 'var(--border)'}` }}
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] mt-0.5" style={{ color: 'var(--text-3)' }}>
                      Created {new Date(t.createdAt).toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric' })}
                      {t.lastUsedAt && ` · Last used ${new Date(t.lastUsedAt).toLocaleDateString('en', { month: 'short', day: 'numeric' })}`}
                      {t.expiresAt && ` · Expires ${new Date(t.expiresAt).toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric' })}`}
                    </p>
                  </div>
                  <button
