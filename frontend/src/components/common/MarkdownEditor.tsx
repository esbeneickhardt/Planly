...
// ReactMarkdown `components` for the preview pane below. Defined at module scope, not inline in
// the component body - nothing here reads a prop or piece of state (every renderer is a pure
// function of its own react-markdown-supplied args), so a fresh object per render would only
// force-remount the whole previewed tree (including any MermaidBlock inside it, causing a visible
// flash/restart) for no benefit. See MessageBubble.tsx's own mdComponents useMemo for the fuller
// explanation of why component identity (not just prop-object identity) matters to react-markdown.
const PREVIEW_MD_COMPONENTS = {
  h1: ({ children }: { children?: React.ReactNode }) => (
    <h1 style={{ fontSize: 18, fontWeight: 700, margin: '0 0 10px' }}>{children}</h1>
  ),
  h2: ({ children }: { children?: React.ReactNode }) => (
    <h2 style={{ fontSize: 15, fontWeight: 600, margin: '14px 0 6px' }}>{children}</h2>
  ),
  ...
  return (
    <div className="space-y-1.5">
      ...
      {preview ? (
        ...
        >
          <ReactMarkdown remarkPlugins={[remarkGfm, remarkBreaks]} components={PREVIEW_MD_COMPONENTS}>
