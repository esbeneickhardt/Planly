...
export default memo(function TaskNode({ data, selected }: NodeProps<TaskNodeData>) {
  ...
  const outerBorder = selected
    ...
    ? '#10b981'
    : isMilestone
    ? (isOverdue ? 'rgba(239,68,68,0.35)' : 'rgba(245,158,11,0.35)')
    : isBlocked
    ? 'rgba(239,68,68,0.35)'
    : 'var(--border)';
  ...
  const bgColor = inSprint
    ...
    : isMilestone
    ? (isOverdue ? 'rgba(239,68,68,0.07)' : 'rgba(245,158,11,0.07)')
    : (isDone ? 'var(--surface-2)' : 'var(--surface)');

  const boxShadow = auraShadow ?? (
    selected
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '10px 12px', paddingRight: inSprint ? 34 : 12 }}>
        ...
        {/* Task name */}
        <p style={{
          fontSize: 13, fontWeight: 500, color: 'var(--text)', lineHeight: 1.3,
