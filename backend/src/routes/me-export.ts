  ...
  app.get('/api/me/export', { preHandler: requireAuth }, async (req, reply) => {
    ...
    const [user, tasks, messages, notifications, apiTokens, announcements, comments, accessRequests, teamMemberships] = await Promise.all([
      prisma.user.findUnique({
        where: { id: userId },
        select: {
          id: true, username: true, email: true, realName: true, phone: true,
          avatarEmoji: true, avatarUrl: true, emailVerified: true, ssoProvider: true,
      ...
      prisma.task.findMany({
        ...
        select: { id: true, name: true, description: true, status: true, createdAt: true, deadline: true, productId: true, ownerId: true, reviewerId: true, createdBy: true },
