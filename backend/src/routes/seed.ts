  ...
  app.post('/api/seed-examples', { preHandler: requireAdmin }, async (req, reply) => {
    ...
    const p1 = await prisma.product.create({
      data: {
        name: 'Podcast Launch',
        emoji: '🎙️',
        description: 'Launch a weekly tech podcast covering AI and software engineering.',
        deadline: p1deadline,
        teamId: team1.id,
        ownerId: userId,
      },
    ...
    const s7 = await t1({
      name: 'Launch social media page',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#f59e0b',
    });

    const sm1deadline = new Date();
    sm1deadline.setDate(sm1deadline.getDate() + 14);
    const sm1 = await t1({
      name: 'Episode 1 Live',
      status: 'backlog',
      ownerId: userId,
      deadline: sm1deadline,
      color: '#f59e0b',
    });
    ...
    await dep(s6.id, s4.id);
    await dep(s6.id, s3.id);
    await dep(s7.id, s3.id);
    await dep(sm1.id, s5.id);
    await dep(sm1.id, s6.id);
    await dep(sm1.id, s7.id);
    ...
    p2deadline.setDate(p2deadline.getDate() + 365);
    const p2 = await prisma.product.create({
      data: {
        name: 'IRB PD Model',
        emoji: '🏦',
        description:
          'Probability of default model for Danish retail customers, compliant with EBA GL 2017/07 and Finanstilsynet IRB requirements.',
        deadline: p2deadline,
        teamId: team2.id,
        ownerId: userId,
      },
    ...
    const t2 = (data: TaskInput) => prisma.task.create({ data: { ...data, productId: p2.id, createdBy: userId } });

    // Phase 1 - Governance & Scope (done)
    const b1 = await t2({
      name: 'Define model scope & governance',
      status: 'done',
      ownerId: userId,
      color: '#7c3aed',
      completedAt: new Date(),
    ...
    const b2 = await t2({
      name: 'Data inventory & gap analysis',
      status: 'done',
      ownerId: demoUser.id,
      color: '#7c3aed',
      completedAt: new Date(),
    ...
    const b3 = await t2({
      name: 'Regulatory requirements mapping (CRR)',
      status: 'done',
      ownerId: userId,
      color: '#7c3aed',
      completedAt: new Date(),
    ...
    const b4 = await t2({
      name: 'Extract data from core banking system',
      status: 'in_progress',
      ownerId: userId,
      color: '#3b82f6',
    });
    const b5 = await t2({
      name: 'Extract external credit bureau data',
      status: 'in_progress',
      ownerId: demoUser.id,
      color: '#3b82f6',
    });
    const b6 = await t2({
      name: 'Data quality assessment & remediation',
      status: 'todo',
      ownerId: userId,
      color: '#3b82f6',
    });
    const b7 = await t2({
      name: 'Define development & validation samples',
      status: 'todo',
      ownerId: demoUser.id,
      color: '#3b82f6',
    });
    ...
    const b10 = await t2({
      name: 'Variable selection (Gini / IV analysis)',
      status: 'backlog',
      ownerId: userId,
      color: '#10b981',
    });
    ...
    const b11 = await t2({
      name: 'Baseline logistic regression model',
      status: 'backlog',
      ownerId: userId,
      color: '#10b981',
    });
    const b12 = await t2({
      name: 'ML challenger model (gradient boosting)',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#10b981',
    });
    const b13 = await t2({
      name: 'Through-the-cycle (TTC) calibration',
      status: 'backlog',
      ownerId: userId,
      color: '#10b981',
    });
    const b14 = await t2({
      name: 'Conservative margin of conservatism',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#10b981',
    });
    const b15 = await t2({
      name: 'Final model selection & tech doc',
      status: 'backlog',
      ownerId: userId,
      color: '#10b981',
    });
    ...
    const b16 = await t2({
      name: 'EBA GL 2017/07 compliance checklist',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#f59e0b',
    });
    const b17 = await t2({
      name: 'Finanstilsynet IRB requirements gap',
      status: 'backlog',
      ownerId: userId,
      color: '#f59e0b',
    });
    const b18 = await t2({
      name: 'MRM framework alignment',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#f59e0b',
    });
    ...
    const b20 = await t2({
      name: 'Regulatory documentation package',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#f59e0b',
    });
    ...
    const b21 = await t2({
      name: 'Discriminatory power testing (ROC/Gini)',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#ef4444',
    });
    ...
    const b23 = await t2({
      name: 'Benchmarking vs external rating agencies',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#ef4444',
    });
    const b24 = await t2({
      name: 'Stress testing under adverse scenarios',
      status: 'backlog',
      ownerId: userId,
      color: '#ef4444',
    });
    const b25 = await t2({
      name: 'Independent model validation report',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#ef4444',
    });
    ...
    const b26 = await t2({
      name: 'Internal model committee approval',
      status: 'backlog',
      ownerId: userId,
      color: '#8b5cf6',
    });
    const b27 = await t2({
      name: 'Finanstilsynet submission package',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#8b5cf6',
    });
    const b28 = await t2({
      name: 'Regulatory Q&A & remediation',
      status: 'backlog',
      ownerId: userId,
      color: '#8b5cf6',
    });
    ...
    const b29 = await t2({
      name: 'Scoring pipeline development',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#06b6d4',
    });
    ...
    const b31 = await t2({
      name: 'UAT & parallel run vs. existing model',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#06b6d4',
    });
    ...
    const b34 = await t2({
      name: 'Automated PSI & Gini reporting',
      status: 'backlog',
      ownerId: userId,
      color: '#10b981',
    });
    const b35 = await t2({
      name: 'Annual review & recalibration process',
      status: 'backlog',
      ownerId: demoUser.id,
      color: '#10b981',
    });
    ...
    const bm2 = await t2({
      name: 'Model Development Complete',
      status: 'backlog',
      ownerId: userId,
      deadline: d120,
      color: '#f59e0b',
    ...
    const bm3 = await t2({
      name: 'IRB Documentation Package',
      status: 'backlog',
      ownerId: demoUser.id,
      deadline: d150,
      color: '#f59e0b',
    ...
    await dep(b4.id, b1.id);
    await dep(b4.id, b2.id);
    await dep(b5.id, b2.id);
    await dep(b6.id, b4.id);
    await dep(b6.id, b5.id);
    ...
    await dep(b15.id, b11.id);
    await dep(b15.id, b12.id);
    await dep(b15.id, b14.id);
    ...
    await dep(b18.id, b17.id);
    await dep(b18.id, bm2.id);
    await dep(b19.id, b14.id);
    await dep(b20.id, b18.id);
    await dep(b20.id, b19.id);
    ...
    await dep(b25.id, b22.id);
    await dep(b25.id, b23.id);
    await dep(b25.id, b24.id);

    // Milestone: Validation Complete
    await dep(bm4.id, b25.id);
    await dep(bm4.id, bm3.id);
    ...
    await dep(b31.id, b30.id);
    await dep(b31.id, bm5.id);
    ...
    const p3deadline = new Date();
    p3deadline.setDate(p3deadline.getDate() + 180);
    const p3 = await prisma.product.create({
      data: {
        name: 'Build a Rocket',
        emoji: '🚀',
        description: 'Design, build, and launch a single-stage rocket. From concept to launch in 180 days.',
        deadline: p3deadline,
        teamId: team3.id,
        ownerId: userId,
      },
    ...
    const rd60 = new Date();
    rd60.setDate(rd60.getDate() + 60);
    const rd80 = new Date();
    rd80.setDate(rd80.getDate() + 80);
    const rm2 = await t3({
    ...
    rd120.setDate(rd120.getDate() + 120);
    const rm4 = await t3({
    ...
    rd160.setDate(rd160.getDate() + 160);
    const rd180 = new Date();
    rd180.setDate(rd180.getDate() + 180);
    const rm5 = await t3({
      name: 'Launch Ready',
      status: 'backlog',
      ownerId: userId,
