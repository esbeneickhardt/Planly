...
export async function totpRoutes(app: FastifyInstance) {
  ...
  app.post('/api/auth/totp/confirm', { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (!verifyCode(user.totpSecret, user.username, body.data.code)) {
  ...
  app.delete('/api/auth/totp/disable', { preHandler: requireAuth }, async (req, reply) => {
    ...
    const codeOk = user.totpSecret
      ? verifyCode(user.totpSecret, user.username, body.data.code)
      : false;
    const backupOk = !codeOk && (await checkBackupCode(user.id, body.data.code));
  ...
  app.post('/api/auth/totp/challenge', async (req, reply) => {
    ...
    try {
      payload = jwt.verify(body.data.mfaToken, config.jwtSecret) as { userId: string; type: string };
    } catch {
    ...
    const codeOk = verifyCode(user.totpSecret, user.username, body.data.code);
    const backupOk = !codeOk && (await checkBackupCode(user.id, body.data.code));
